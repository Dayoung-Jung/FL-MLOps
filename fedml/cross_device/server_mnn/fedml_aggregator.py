...
class FedMLAggregator(object):
    ...
    def test(self, test_data, device, args):
        ...
        if self.args.enable_wandb:
            mlflow.log_metrics({"round " + str(args.round_idx) + " Test/Acc": test_acc})
            mlflow.log_metrics({"round " + str(args.round_idx) + " Test/Loss": test_loss})
    ...
    def test_on_server_for_all_clients(self, round_idx, global_model_file=None):
        if round_idx % self.args.frequency_of_the_test == 0 or round_idx == self.args.comm_round - 1:
            ...
            if round_idx == self.args.comm_round - 1:
                # mlflow.log_params({"round_idx": round_idx})
    ...
    def test_on_server_for_all_clients_mnn(self, mnn_file_path, round_idx, report_metrics=True):
        ...
        if report_metrics:
            ...
            mlflow.log_metrics(
                {
                    "round " + round_idx + " accuracy": round(float(np.round(test_accuracy, 4)), 4),
                    "round " + round_idx + " loss": round(float(np.round(test_loss, 4)), 4),
                }
            )
            #
            # mlops.log(
            ...
            if self.args.enable_wandb:
                mlflow.log_metrics(
                    {
                        "round " + str(round_idx) + " test acc": test_accuracy,
                        "round " + str(round_idx) + " test loss": test_loss,
                    }
